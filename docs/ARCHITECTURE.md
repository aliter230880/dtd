...
**Версия:** 1.1  
**Статус проекта:** MVP работает на всех платформах (Android/iOS/Web), веб-сборка проходит полный цикл сделки
...
Раздел отражает состояние на **24 августа 2026**. Ниже — только то, что проверено
...
кэши pub и gradle. Повторная сборка занимает 2–3 минуты.
...
3. **Кошелёк.** RevenueCat при `kIsWeb` не инициализируется, страница висела на
   спиннере. В браузере теперь свой экран пополнения через Stripe Checkout.
...
Статусы актуальны на 24 августа 2026.
