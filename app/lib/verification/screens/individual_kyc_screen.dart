...
class IndividualKycScreen extends StatefulWidget {
  /// true — экран открыт отдельным маршрутом: нужен свой Scaffold и стрелка
  /// назад. false — вкладка внутри VerificationScreen.
  final bool standalone;

  const IndividualKycScreen({super.key, this.standalone = false});
  ...
  Widget _layerCard(KycLayer layer) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: done ? accent.withOpacity(0.45) : T.divider,
          width: done ? 1.4 : 1,
        ),
      ),
      child: Column(
