...
class CarrierDealStatusCompWidget extends StatefulWidget {
  const CarrierDealStatusCompWidget({
    super.key,
  ...
  @override
  State<CarrierDealStatusCompWidget> createState() => _CarrierDealStatusCompWidgetState();
}

class _CarrierDealStatusCompWidgetState extends State<CarrierDealStatusCompWidget> {
          ...
          border: Border.all(
            color: () {
              if (widget.status == DealStatus.InSearch) {
                return const Color(0xFFA9A9AA);
              } else if (widget.status == DealStatus.InConfirm) {
                return const Color(0xFF7812B6);
        ...
        child: Align(
          ...
          child: Padding(
            ...
            child: Text(
              () {
                if (widget.status == DealStatus.InSearch) {
                  return FFLocalizations.of(context).getText('carrier_status_in_search');
                } else if (widget.status == DealStatus.InConfirm) {
                  return FFLocalizations.of(context).getText('carrier_status_in_confirm');
                } else if (widget.status == DealStatus.InActive) {
