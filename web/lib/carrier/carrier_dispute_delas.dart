...
class CarrierDisputeDealsWidget extends StatefulWidget {
  final int length;
  const CarrierDisputeDealsWidget({super.key, this.length = 0});

  @override
  State<CarrierDisputeDealsWidget> createState() => _CarrierDisputeDealsWidgetState();
}

class _CarrierDisputeDealsWidgetState extends State<CarrierDisputeDealsWidget> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          '${FFLocalizations.of(context).getText('diller_status_in_dispute')} (${widget.length})',
                ...
                child: StreamBuilder<List<DealsRecord>>(
                  stream: queryDealsRecord(
                    queryBuilder: (dealsRecord) => dealsRecord
                        .where('carrier', isEqualTo: currentUserReference)
                        .whereIn('status', [DealStatus.InDispute.name]).orderBy('created_time', descending: true),
                  ),
