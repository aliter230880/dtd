                    ...
                    return Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          FFButtonWidget(
                            onPressed: () async {
                              if (filesItem.isEmpty) {
                                final selectedFiles = await selectFiles(multiFile: false);

                                final Uint8List? bytes = selectedFiles?.first.bytes;

                                if (bytes != null) {
                                  setState(() {
                                    FFAppState().updateCreatDealFilesAtIndex(filesIndex, (_) => bytes);
                                  });
                                }
                              } else {
                                setState(() {
                                  FFAppState().updateCreatDealFilesAtIndex(filesIndex, (_) => Uint8List.fromList([]));
                                });
                              }
                            },
                            text: (filesItem.isNotEmpty) ? 'Удалить' : 'Загрузить',
                            options: FFButtonOptions(
                              width: 111.0,
                              height: 36.0,
                              padding: const EdgeInsetsDirectional.fromSTEB(0.0, 0.0, 0.0, 0.0),
                              iconPadding: const EdgeInsetsDirectional.fromSTEB(0.0, 0.0, 0.0, 0.0),
                              color: filesItem.isNotEmpty
