...
class _CreateDealPageWidgetState extends State<CreateDealPageWidget> {
  ...
  Future<void> onCreateDeal() async {
    try {
      ...
      final List<Uint8List> carFiles = FFAppState().creatDealFiles.where((p) => p.isNotEmpty).toList();

      final carFilesUrl = (await Future.wait(carFiles.map((m) async => await uploadToDBPath2(m))));
